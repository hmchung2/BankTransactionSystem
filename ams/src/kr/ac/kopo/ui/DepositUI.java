...
public class DepositUI extends BaseUI {
	...
	public void execute() throws Exception {
		...
		while (true) {
			...
			String accountId = scanStr("입금할 계좌 번호를 입력 하세요 : ");
			int money = scanInt("입금액을 입력하세요 : ");
			Map<String, String> result = service.deposit(accountId, money);
			if (result.get("success").equals("true")) {
				System.out.println("입금 완료 했습니다. 입금 시스템 종료 합니다.");
				break;
			} else {
				if (result.get("accountExist").equals("false")) {
					...
					System.out.println("계좌 입금중 에러 발생");
				...
				System.out.println("입금을 다시 하시겠습니까?");
